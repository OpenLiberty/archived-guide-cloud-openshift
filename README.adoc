...
A cloud-based infrastructure enables you to focus on developing your microservices without worrying about 
low-level infrastructure details for deployment. Using a cloud helps
...
OpenShift is a {kube}-based platform with added functionality. It streamlines the DevOps process by providing an intuitive development pipeline, 
...
Creating an OpenShift account will automatically grant you access to their multi-tenant, OpenShift cluster. 
...
You will need to create a project by simply running the following command:
...
Once pushed, run the following command to list the images pushed to the internal OCR:
[role=command]
```
oc get imagestream
```
...
The hostnames will be formatted like so `inventory-route-[project-name].apps.[region].starter.openshift-online.com`. Ensure that
