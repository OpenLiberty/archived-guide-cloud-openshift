...
:page-description: Explore how to deploy microservices to Red Hat OpenShift's public cloud. 
...
Explore how to deploy microservices to Red Hat OpenShift's public cloud. 
...
cluster on OpenShift Online, Red Hat's public cloud platform.
...
you to easily scale and serve your microservices in a high-availability setup. This guide will use OpenShift Online, 
where you will host your own {kube} cluster on Red Hat's pubilc cloud, and deploy your containerized applications to.
To learn more about the different platforms that Red Hat OpenShift offers, check out their  
...
https://www.openshift.com/trial/[official website^]. Signing up for an account will automatically provision a single
cluster for you to use.
...
Pushing the images to a registry allows the cluster to create pods by using your container images. The registry that you 
will use is called OpenShift Container Registry (OCR).
...
However, if you are using a custom credentials store for securing your registry credentials, you will need to use the following command:
...
You can also view the registry address by running the following:
...
Ensure you keep note of the `[region]` section, as it will be needed in further steps.
...
remember to substitute `[region]` and `[project-name]` with the appropriate values:

[role=command]
```
docker tag system:1.0-SNAPSHOT default-route-openshift-image-registry.apps.[region].online-starter.openshift.com/[project-name]/system:1.0-SNAPSHOT
docker tag inventory:1.0-SNAPSHOT default-route-openshift-image-registry.apps.[region].online-starter.openshift.com/[project-name]/inventory:1.0-SNAPSHOT
```
...
[role=command]
```
docker push default-route-openshift-image-registry.apps.[region].online-starter.openshift.com/[project-name]/system:1.0-SNAPSHOT 
docker push default-route-openshift-image-registry.apps.[region].online-starter.openshift.com/[project-name]/inventory:1.0-SNAPSHOT
```
...
to use for the container. Update the system [hotspot=systemImage]`image` and the inventory [hotspot=inventoryImage]`image` 
fields to include your project name.
...
Your microservices can now be accessed through the hostnames which can be found in the web console under `Networking > Routes > Location`. 
The hostnames will be formatted like so `inventory-service-[project-name].apps.[region].online-starter.openshift.com`. Ensure
you are in your project, not the default, which is shown in the top left corner of the web console.
...
Point your browser to the `http://[inventory-hostname]/inventory/systems/system-service` URL. When you visit this URL, these system
properties are automatically stored in the inventory. Go back to `[inventory-hostname]/inventory/systems` and
you see a new entry.
...
`[cluster-ip]` and `[project-name]` with the appropriate values:

[role=command]
```
mvn verify -Ddockerfile.skip=true \
-Dsystem.ip=system-service-[project-name].apps.[cluster-ip].online-starter.openshift.com  \
-Dinventory.ip=inventory-service-[project-name].apps.[cluster-ip].online-starter.openshift.com
```
...
by running the `oc delete` command:
...
You have just deployed two microservices running in Open Liberty to OpenShift Online. You also 
