...
Hostnames are in the `inventory-route-guide.apps.[region].starter.openshift-online.com` format. Ensure that
...
`[region]` with the appropriate values:

[role=command]
```
mvn verify \
-Dsystem.ip=system-route-guide.apps.[region].starter.openshift-online.com  \
-Dinventory.ip=inventory-route-guide.apps.[region].starter.openshift-online.com
```
