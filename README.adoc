...
you to easily scale and serve your microservices in a high-availability setup. You will build and
deploy containerized applications to your own {kube} cluster on OpenShift.
...
- *OpenShift account:* To create a {kube} cluster, you must sign up for a Red Hat account, and select the OpenShift Online starter tier, which
...
cluster for you to use. Keep in mind that the creation time depends on resource availability, and may take some time.
...
Creating a starter tier OpenShift account will automatically provision a cluster for you. Once your cluster is provisioned, 
you are given access to their online web console. To login to OpenShift using the CLI, 
...
You should also use the above command if you run into authentication errors when pushing the images further down the line.

****
[system]#*{win}*#

Since the Windows Command Prompt doesn't support the command substitution as displayed, you must run the `oc whoami`, `oc whoami -t`,
and `oc registry info` commands seperately and then substitute them manually into the `docker login` command.
****
