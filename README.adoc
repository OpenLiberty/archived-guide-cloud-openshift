...
To learn how to deploy to an OpenShift 4 cluster,
see the https://openliberty.io/guides/cloud-openshift-operator.html[Deploying microservices to OpenShift by using Kubernetes Operators^] guide. 
...
If you would like to learn more about Kubernetes,
check out the https://openliberty.io/guides/kubernetes-intro.html[Deploying microservices to Kubernetes^] guide.

OpenShift is a Kubernetes-based platform with added functions.
It streamlines the DevOps process by providing an intuitive development pipeline. 
...
check out their https://docs.openshift.com[official documentation^]. 
...
* *OpenShift account:* To access a {kube} cluster, you must sign up for a Red Hat OpenShift Online account.
There are two options, Starter and Pro. 
Use the Starter plan, which includes a free trial of the OpenShift platform with limited resources,
making it perfect for individual experimentation. 
...
you need to push the microservice images into a container image registry. 
...
If you are using a custom credential store for securing your registry credentials,
or if you are unsure where your credentials are stored, use the following command:
...
The registry address will be displayed after you run the `oc registry login` command.
...
When you to go this URL, the system properties that are taken from the `system`
service are automatically stored in the inventory.
