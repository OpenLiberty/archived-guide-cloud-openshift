...
deploy containerized applications to an OpenShift cluster. 

OpenShift is a Kubernetes-based platform with added functionality. To learn more about the different platforms that Red Hat 
OpenShift offers, check out their https://docs.openshift.com[official documentation^]. Kubernetes is an open source container orchestrator that a
utomates many tasks that are involved in deploying, managing, and scaling containerized applications. If you would like to learn
...
will use is OpenShift's own internal registry, called OpenShift Container Registry (OCR), which provices a built-in
