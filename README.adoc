...
[role="no_copy"]
----
default-route-openshift-image-registry.apps.[cluster-id].online-starter.openshift.com
----
...
Ensure you keep note of the `[cluster-id]` section, as it will be needed in further steps.
...
remember to substitute `[cluster-id]` and `[project-name]` with the appropriate values:
...
The hostnames will be formatted like so `inventory-service-[project-name].apps.[cluster-id].online-starter.openshift.com`. Ensure
