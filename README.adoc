...
To learn how to deploy to an OpenShift 4 cluster, see the https://openliberty.io/guides/cloud-openshift-operator.html[Deploying microservices to OpenShift by using Kubernetes Operators^] guide. 
...
https://docs.openshift.com/container-platform/3.11/getting_started/install_openshift.html[OpenShift Online documentation^].
...
Create a project by running the following command. replacing `[project-name]` with a name of your choice:
...
When you to go this URL, the system properties that are taken from the `system` service are automatically stored in the inventory. 
...
Finally, you can delete the project by running the following command, replacing `[project-name]` with the name you selected earlier:
